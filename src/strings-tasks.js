...
function removeFirstOccurrences(str, value) {
  return str.indexOf(value).toString().slice(90).concat(str.replace(value, ''));
}
...
function removeLastOccurrences(str, value) {
  ...
  return str
    ...
    .toString()
    .slice(90)
    .concat(strReverse.replace(valueReverse, '').split('').reverse().join(''));
}
...
function countVowels(str) {
  const vowelsList = 'aeyuio';
  return str.split('').filter((char) => vowelsList.includes(char.toLowerCase()))
    .length;
}
...
function isPalindrome(str) {
  const regExp = /\*|!|\?|,| |/g;
  const cleanStr = str.toLowerCase().replaceAll(regExp, '');
...
function reverseWords(str) {
  ...
  return strReverse.map((word) => word.split('').reverse().join('')).join(' ');
}
...
function encodeToRot13(str) {
  ...
  const encodeStr = str.replace(/[a-z]/gi, (char) => {
    return alphabet[13 + alphabet.indexOf(char)];
  });
  return encodeStr;
}
